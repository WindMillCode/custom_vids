...
declare global {
    ...
    var mediaPrefix
    var getDocHeight
    var latchUtilities:{centerX:Function,boardPercentage:Function}
}
...
export var latchUtilities:{centerX:Function,boardPercentage:Function} ={
    // buggy on production
    ...
    // }
    //
    boardPercentage:(devObj)=>{
        let {zChildren,zSymbol,metadata} = devObj
        let percent = metadata?.percent || .4
        return numberParse(getComputedStyle(zChildren["&#8353"].element).width) * percent
    },
    centerX:(devObj)=>{
        let {zChildren,zSymbol,metadata} = devObj
...
// diretive fns
export let judimaDirective:{
    onInit:Function,
    onDestroy:Function
} = {
    onInit:(devObj)=>{
        let {myThis,featureFn} = devObj
        let {extras,appExtras,ryber,zChildren, subscriptions} = myThis
        let { co } = extras
        let { group, suffix } = ryber[co].metadata[appExtras.name]
        if (env.directive?.[appExtras.name]?.lifecycleHooks) console.log(extras.co + " " + extras.zSymbol + " " + appExtras.name + ' Directive ngInit fires on mount')
        ...
        .subscribe((result: any) => {
            ...
            let ref = result[0].ref || myThis.ref
            ...
            Object.entries(zChildren)
                ...
                .forEach((x: any, i) => {
                    ...
                    if (x[1].extras?.[appExtras.selector] === undefined) {
                        return
                    }
                    //
                    ...
                    let myGroup:string = x[1].extras[appExtras.selector]?.group || "default"
                    let myType: Array<string> = x[1].extras[appExtras.selector]?.type || ["default"]
                    ...
                    let count = 0
                    if (x[1].extras[appExtras.selector]?.duplicateIgnore !== "true") {
            ...
            featureFn({group,subscriptions,navAction})
            //
    ...
    onDestroy:(devObj)=>{
        let {myThis} = devObj
        let {extras,appExtras} = myThis
        if(extras?.confirm === "true"){
            if (env.directive?.[appExtras.name]?.lifecycleHooks) console.log(extras.co + " " + extras.zSymbol + " " + appExtras.name + ' Directive ngOnDestroy fires on dismount')
            myThis.subscriptions
                .forEach((x: any, i) => {
                    try {
                        x.unsubscribe()
                    }
